(function () {
  ...
  function upcomingDemo(ex) {
    // Mientras se graban las demostraciones, el miembro ve el lugar y el formato
    // vertical del video con el nombre del ejercicio, no una caja vacía.
    return '<button class="portrait-play demo-soon" type="button" data-demo-soon aria-label="Demostración de ' + esc(ex.name) + ': se publica pronto"><span class="stage-label">VIDEO EN PRODUCCIÓN</span><span class="play-circle">' + icon('play') + '</span><strong>' + esc(ex.name) + '</strong><span class="small">Demostración en formato vertical.<br>Se publica pronto.</span></button>';
  }
  ...
  function unlockText(module) {
    // El contenido se publica por fecha: una fecha concreta se entiende mejor que «a los 2 días».
    const at = new Date(module.unlockAt || '');
    if (Number.isNaN(at.getTime())) return 'Se abre a los ' + module.unlockAfterDays + ' días de confirmar tu registro.';
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const day = new Date(at); day.setHours(0, 0, 0, 0);
    const days = Math.round((day - today) / 86400000);
    const time = at.toLocaleTimeString('es-419', { hour: 'numeric', minute: '2-digit' });
    if (days <= 0) return 'Se abre hoy a las ' + time + '.';
    if (days === 1) return 'Se abre mañana a las ' + time + '.';
    return 'Se abre el ' + at.toLocaleDateString('es-419', { weekday: 'long' }) + ' ' + at.toLocaleDateString('es-419', { day: 'numeric', month: 'long' }) + '.';
  }
  function render() {
  ...
  document.addEventListener('click', event => {
    ...
    } else if (target.hasAttribute('data-demo-soon')) toast('La demostración de este ejercicio se publica pronto. Mientras tanto, sigue las series, repeticiones y descansos indicados.');
  ...
  document.addEventListener('visibilitychange', () => {
    // Si al volver a la app un módulo ya llegó a su fecha, se recarga para traer su contenido.
    if (!document.hidden && D.modules.some(m => m.locked && Date.parse(m.unlockAt || '') <= Date.now())) location.reload();
  });
